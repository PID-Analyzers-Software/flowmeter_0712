...
void SSD1327RunMenuRenderer::render(Menu* menu)
{
  ...
  m_display->drawStr(100, 30, String(String(( m_dataSource->getRawMiliVolts_battery() - 3000) / 4) + "%").c_str());
...
void SSD1327SleepTimerMenuRenderer::render(Menu* menu)
{
  ...
  if (m_sleepTimer->getSelectedInterval() == -1) {
    m_display->drawStr(10, 60, "Continous");
  } else {
    m_display->drawStr(10, 60, String(String(m_sleepTimer->getSelectedInterval()) + " min").c_str());

  }
  m_display->drawStr(10, 90, "[S] Next");
